...
class pintheon_payload:
    ...
    """
    def __init__(self, pintheonOptions, name, filePath, wrapWithDirectory, pinToIPFS, pintheonMetadata):
...
class W3DeployHandler:
    ...
    def build_access_token(self):
        ...
        if self.access_token != '':
            ...
            self.access_token = StellarSharedKeyTokenBuilder(self.keys_25519, self.app_pub25519).serialize()
            ...
            with open(file_path, 'rb') as f:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        raise UploadError(f"Invalid JSON response from server: {response.text}")
                    ...
                    if cid:
                        ...
                        # Basic gateway format validation
                        if '://' in gateway or ' ' in gateway or len(gateway) > 255:
                            raise ConfigurationError(f"Invalid gateway format: {gateway}")
                        ...
                        raise UploadError(f"Upload successful but no CID returned: {response.text}")
                else:
                    raise UploadError(f"Upload failed with status {response.status_code}: {response.text}")
                    
        except (ValueError, TypeError, FileNotFoundError, UploadError, ConfigurationError):
    ...
    def pintheonCss(self, filePath):
        ...
        payload = pintheon_payload('{"cidVersion": 1}', fileName, filePath, 'true', 'true', metadata)
    ...
    def uploadFilesIndividually(self, files, payload, window=None):
        ...
        if not self.pintheon['access_token']:
            print('Pintheon Access Token Not Set!')
            return type('Response', (), {'status_code': 401, 'text': 'Access token not set'})()
        
        if not self.pintheon['gateway']:
            print('Pintheon Gateway Not Set!')
            return type('Response', (), {'status_code': 400, 'text': 'Gateway not set'})()
        ...
        if failed_files:
            print(f"Failed to upload {len(failed_files)} files: {failed_files}")
            return type('Response', (), {'status_code': 500, 'text': f'Failed to upload {len(failed_files)} files'})()
        else:
            print(f"Successfully uploaded {len(uploaded_files)} files")
            return type('Response', (), {'status_code': 200, 'json': lambda: {'IpfsHash': 'multiple_files_uploaded'}})()
    
    def uploadMediaFolder(self, filePath, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False):
        """
    ...
    def uploadMediaFolderGUI(self, filePath, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False, askPermission=True):
        """
        ...
        if askPermission:
            ...
            popup_result = sg.popup_yes_no('Deploy Media Files to IPFS via Pintheon?', 
        ...
        if self.pintheon['access_token'] and self.pintheon['gateway']:
            # Call the new method
            success = self.uploadMediaFolder(filePath, wrapWithDirectory, pinToIPFS, useParentDirs)
            if success:
    ...
    def uploadResourcesGUI(self, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False):
        """
        ...
        :rtype: (tuple) or (None)
        """
        return self.uploadMediaFolderGUI(self.resourcePath, wrapWithDirectory, pinToIPFS, useParentDirs)
    ...
    def pintheonDirectory(self, filePath, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False):
        """Legacy method - use uploadMediaFolder instead."""
        return self.uploadMediaFolder(filePath, wrapWithDirectory, pinToIPFS, useParentDirs)
    
    def pintheonDirectoryGUI(self, filePath, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False, askPermission=True):
        """Legacy method - use uploadMediaFolderGUI instead."""
        return self.uploadMediaFolderGUI(filePath, wrapWithDirectory, pinToIPFS, useParentDirs, askPermission)
    
    def pintheonResourcesGUI(self, wrapWithDirectory=True, pinToIPFS=True, useParentDirs=False):
    ...
    def _folderArray(self, key, filePath, basePath, window=None):
        ...
        print("Warning: _folderArray is deprecated and not needed for individual file uploads")
        pass
