...
class LoadingWindow:
   ...
   def __init__(self):
      ...
      self.bar_layout = [[sg.Text('', size=(self.bar_len, 1), relief='sunken', text_color='yellow', background_color='black',key='-TEXT-', metadata=0)]]
      self.running = True
      self.window = None
      self.threaded = False
      self.thread = None
   ...
   def launchBar(self, methods, *args):
         ...
         self.window = sg.Window('Title', self.bar_layout, finalize=True)
