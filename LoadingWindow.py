...
class LoadingWindow:
    ...
    def launchWheel(self, method=None, *args):
        ...
        if method != None:
            ...
            self.thread = threading.Thread(
                target=self.callback, args=(method, args), daemon=True
            )
        self.thread.start()
        self.active = True
        ...
        while self.running and self.active and self.thread.is_alive():  # Event Loop
            event, values = self.window.read(
                timeout=10
            )  # loop every 10 ms to show that the 100 ms value below is used for animation
            if event == "Exit":
                break
            if event in (sg.WIN_CLOSED, "Exit", "Cancel"):
                break
            # update the animation in the window
            self.window["-START-IMAGE-"].update_animation(
                self.gif, time_between_frames=100
            )
    ...
    def launchBar(self, methods, *args):
        ...
        self.window = sg.Window(
            "Loading", self.bar_layout, keep_on_top=True, finalize=False
        )

        self.threaded = True
        self.thread = threading.Thread(
            target=self.bar_callback, args=(methods, args), daemon=True
        )
        self.thread.start()
        self.active = True
        ...
        while self.running and self.active and self.thread.is_alive():
            ...
            if event in (sg.WIN_CLOSED, "Exit", "Cancel"):
                print("should be exited!!")
                break
            self.window["-BAR-IMAGE-"].update_animation(
                self.gif, time_between_frames=100
            )
