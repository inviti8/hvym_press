...
class MarkdownHandler:
    ...
    def _deployedURL(self, href):
        ...
        print(
            f"DEBUG: Looking for {decoded_filename} (decoded from {filename}) in manifest with {len(media_files)} files"
        )
        ...
        for media_filename, media_data in media_files.items():
            if media_filename.lower() == decoded_filename.lower():
                ...
                print(
                    f"DEBUG: Case-insensitive match found for {decoded_filename}: {deployed_url}"
                )
                print(
                    f"Found media file {decoded_filename} in manifest (case-insensitive match)"
                )
                return deployed_url
        ...
        for media_filename, media_data in media_files.items():
            ...
            if media_filename_no_ext.lower() == decoded_filename_no_ext.lower():
                deployed_url = media_data["url"]
                print(
                    f"DEBUG: Extension-insensitive match found for {decoded_filename}: {deployed_url}"
                )
                print(
                    f"Found media file {decoded_filename} in manifest (extension-insensitive match)"
                )
                return deployed_url
            ...
            if "href" in link.attrs:
                ...
                # Handle audio files
                    ...
                    if deployed_href != href:
                        ...
                        new_tag = soup.new_tag(
                            "audio", controls=None, src=deployed_href, type="audio/mpeg"
                        )
        ...
        for img in imgs:
            if "src" in img and f"../{self.mediaDir}/" in img["src"]:
                img["src"] = img["src"].replace(
                    f"../{self.mediaDir}/", f"./{self.mediaDir}/"
                )

        for link in links:
            if (
                "src" in link
                and (".mp3" in link["src"] or ".mp4" in link["src"])
                and f"../{self.mediaDir}/" in link["src"]
            ):
    ...
    def _flattenMediaLinks(self, html, resource_dir):
        ...
        for link in links:
            if (
                "src" in link and (".mp3" in link["src"] or ".mp4" in link["src"])
            ) and ("src" in img and f"../{resource_dir}/" in img["src"]):
    ...
    def renderPageTemplate(self, template_file, data, page):
        ...
        if f"../{self.mediaDir}/" in output or f"{self.mediaDir}/" in output:
            ...
            if media_links:
                print(
                    f"DEBUG: Found local media links in template: {media_links[:3]}..."
                )
        else:
